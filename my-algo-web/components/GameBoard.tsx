...
export default function GameBoard({
  ...
  const isCpu = opponent?.isCpu || opponentName === "CPU";
          ...
          <div className="flex gap-2 sm:gap-4 flex-wrap justify-center">
            {gameState.opponentHand.map((card) => (
              <CardView
                ...
                disabled={!isMyTurn || card.isOpen || isProcessing || !isConnected}
                guessedNumber={
                  lastAttack &&
                  !lastAttack.isYourCard &&
                  lastAttack.targetCardId === card.id
                    ? lastAttack.guess
                    : undefined
                }
              />
          ...
          <div className="w-14 h-20 flex items-center justify-center relative">
            {gameState.drawnCard ? (
              ...
              <div className="w-full h-full border-2 border-dashed border-slate-200 rounded-xl flex items-center justify-center bg-white/50">
                <span className="text-[10px] text-slate-300 font-bold">FIELD</span>
              </div>
            )}
          </div>
